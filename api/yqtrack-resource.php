...
class YqTrack_Resource
{
	protected $server;
	protected $base;
	
	public function __construct(YqTrack_Server $server)
	{
		$this->server = $server;
		add_filter('yqtrack_api_endpoints', array($this, 'register_routes'));
		foreach (array('order', 'coupon', 'customer', 'product', 'report') as $resource) {
			add_filter("yqtrack_api_{$resource}_response", array($this, 'maybe_add_meta'), 15, 2);
			add_filter("yqtrack_api_{$resource}_response", array($this, 'filter_response_fields'), 20, 3);
		}
	}
	...
	protected function validate_request($id, $type, $context)
	{
		...
		$id = absint($id);
		if (empty($id))
			return new WP_Error("yqtrack_api_invalid_{$resource_name}_id", sprintf(__('Invalid %s ID', 'yqtrack'), $type), array('status' => 404));

		if ('customer' !== $type) {
			$post = get_post($id);
			$post_type = ('product_variation' === $post->post_type) ? 'product' : $post->post_type;
			if ($type !== $post_type)
				return new WP_Error("yqtrack_api_invalid_{$resource_name}", sprintf(__('Invalid %s', 'yqtrack'), $resource_name), array('status' => 404));
			switch ($context) {
				case 'read':
					if (!$this->is_readable($post))
						return new WP_Error("yqtrack_api_user_cannot_read_{$resource_name}", sprintf(__('You do not have permission to read this %s', 'yqtrack'), $resource_name), array('status' => 401));
					break;
				case 'edit':
					if (!$this->is_editable($post))
						return new WP_Error("yqtrack_api_user_cannot_edit_{$resource_name}", sprintf(__('You do not have permission to edit this %s', 'yqtrack'), $resource_name), array('status' => 401));
					break;
				case 'delete':
					if (!$this->is_deletable($post))
						return new WP_Error("yqtrack_api_user_cannot_delete_{$resource_name}", sprintf(__('You do not have permission to delete this %s', 'yqtrack'), $resource_name), array('status' => 401));
					break;
			}
		}
		return $id;
	...
	protected function delete($id, $type, $force = false)
	{
		...
		if ('customer' === $type) {
			...
			if ($result)
				return array('message' => __('Permanently deleted customer', 'yqtrack'));
			else
				return new WP_Error('yqtrack_api_cannot_delete_customer', __('The customer cannot be deleted', 'yqtrack'), array('status' => 500));
		} else {
			$result = ($force) ? wp_delete_post($id, true) : wp_trash_post($id);
			if (!$result)
				return new WP_Error("yqtrack_api_cannot_delete_{$resource_name}", sprintf(__('This %s cannot be deleted', 'yqtrack'), $resource_name), array('status' => 500));

			if ($force) {
				return array('message' => sprintf(__('Permanently deleted %s', 'yqtrack'), $resource_name));

			} else {
				$this->server->send_status('202');
				return array('message' => sprintf(__('Deleted %s', 'yqtrack'), $resource_name));
			}
		}
	}
